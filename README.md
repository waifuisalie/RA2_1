...
├── AnalisadorSintatico.py             # ★ Analisador sintático principal - integra RA1 + RA2
├── arvore_output.txt                   # Saída da árvore sintática da última execução
│
├── teste1.txt                          # ★ Arquivo de teste 1 - operações básicas (raiz)
├── teste2.txt                          # ★ Arquivo de teste 2 - estruturas de controle (raiz)
├── teste3.txt                          # ★ Arquivo de teste 3 - casos complexos (raiz)
├── teste4_incorreto.txt                # ★ Arquivo de teste 4 - casos com erros sintáticos (raiz)
│
├── src/                                # Código-fonte modular organizado por fase
│   ├── RA1/                           # ★ Fase 1 - Analisador Léxico (reutilizado)
│   │   └── functions/
│   │       ├── python/                # Funções Python do analisador léxico
│   │       │   ├── __init__.py
│   │       │   ├── analisador_lexico.py      # Tokenização e análise léxica
│   │       │   ├── tokens.py                 # Definições e tipos de tokens
│   │       │   ├── io_utils.py               # Utilitários de entrada/saída
│   │       │   ├── rpn_calc.py               # Calculadora RPN (execução)
...
│   │           ├── __init__.py
│   │           ├── builder.py                # Construtor principal de assembly
│   │           ├── header.py                 # Cabeçalho do programa assembly
│   │           ├── footer.py                 # Rodapé do programa assembly
│   │           ├── data_section.py           # Seção .data (variáveis)
│   │           ├── code_section.py           # Seção .text (código)
│   │           ├── operations.py             # Operações aritméticas
│   │           ├── registers.py              # Gerenciamento de registradores
│   │           ├── routines.py               # Rotinas auxiliares
│   │           └── io.py                     # Entrada/saída assembly
│   │
│   └── RA2/                           # ★ Fase 2 - Analisador Sintático LL(1)
│       └── functions/
│           └── python/                # Funções principais do parser LL(1)
│               ├── __init__.py
│               ├── lerTokens.py              # ★ Função 1: Leitura e processamento de tokens
│               ├── construirGramatica.py     # ★ Função 2: Construção da gramática LL(1)
│               ├── parsear.py                # ★ Função 3: Parser descendente recursivo
│               ├── gerarArvore.py            # ★ Função 4: Geração da árvore sintática
│               ├── configuracaoGramatica.py  # Configuração central da gramática
│               ├── calcularFirst.py          # Cálculo dos conjuntos FIRST
│               ├── calcularFollow.py         # Cálculo dos conjuntos FOLLOW
│               └── construirTabelaLL1.py     # Construção da tabela de parsing LL(1)
│
├── inputs/                             # Arquivos de entrada para testes
│   ├── RA1/                           # Entradas específicas do RA1
│   │   ├── float/                     # Testes com números de ponto flutuante
│   │   │   ├── teste1.txt
│   │   │   ├── teste2.txt
│   │   │   ├── teste3.txt
│   │   │   └── teste_parenteses_completo.txt
│   │   └── int/                       # Testes com números inteiros (assembly)
│   │       ├── teste1_assembly.txt
│   │       ├── teste2_assembly.txt
│   │       ├── teste3_assembly.txt
│   │       └── teste_parenteses_completo.txt
│   └── RA2/                           # Entradas específicas do RA2
│       ├── teste1.txt                        # Operações básicas + estruturas
│       ├── teste2.txt                        # Estruturas de controle avançadas
│       ├── teste3.txt                        # Casos complexos e aninhados
│       └── teste4_incorreto.txt              # Casos com erros sintáticos
│
├── outputs/                            # Arquivos de saída gerados
│   ├── RA1/                           # Saídas do analisador léxico
│   │   ├── tokens/
│   │   │   └── tokens_gerados.txt            # Tokens gerados pelo analisador léxico
│   │   └── assembly/
│   │       └── programa_completo.S           # Código assembly RISC-V gerado
...
│   │   └── Github/
│   │       └── README.md                     # Documentação original do RA1
│   └── RA2/                           # Documentação da Fase 2
│       ├── Github/
│       │   └── README.md                     # Documentação específica do RA2
│       └── documents/                 # Documentação técnica detalhada
│           ├── github_issues_workflow.md     # Processo de colaboração da equipe
│           ├── new_tokens_specification.md   # Especificação dos novos tokens
│           ├── 01_Grammar_Fundamentals.md    # Fundamentos de gramática
...
│           ├── Comprehensive_LL1_Grammar_Specification.md
│           └── Corrected_LL1_Grammar_Documentation.md
│
└── flowcharts/                         # Diagramas e fluxogramas
    └── RA2/                           # Arquitetura do RA2
        ├── overview_flowchart.md             # Fluxograma geral do sistema
        ├── code_flowchart.md                 # Fluxograma do código
        ├── RA2_Architecture_Overview.md      # Visão geral da arquitetura
        └── RA2_Function_Interfaces.md        # Interfaces entre funções
```

### Legenda
- **★** = Arquivos principais obrigatórios para entrega
- **RA1/** = Reutilização da Fase 1 (Analisador Léxico)
- **RA2/** = Implementação da Fase 2 (Analisador Sintático LL(1))

### Notas Importantes

1. **Arquivos de Teste na Raiz**: Conforme especificação do PDF, os arquivos de teste (`teste1.txt`, `teste2.txt`, `teste3.txt`, `teste4_incorreto.txt`) devem estar **no mesmo diretório do código-fonte** (`AnalisadorSintatico.py`)

2. **Estrutura Modular**: O código está organizado em módulos separados por fase:
   - `src/RA1/`: Código reutilizado da Fase 1 (Analisador Léxico)
   - `src/RA2/`: Código novo da Fase 2 (Analisador Sintático)

3. **Integração RA1 + RA2**: O arquivo `AnalisadorSintatico.py` na raiz integra ambas as fases:
   - Utiliza funções de tokenização do RA1
   - Executa análise sintática do RA2
   - Gera árvore sintática como saída final

4. **Organização de Entradas/Saídas**:
   - `inputs/`: Arquivos de teste organizados por fase
   - `outputs/`: Resultados gerados (tokens, assembly, árvores sintáticas)

5. **Documentação Completa**:
   - `docs/RA2/documents/`: Documentação técnica detalhada da gramática, FIRST/FOLLOW, tabela LL(1)
   - `flowcharts/RA2/`: Diagramas de arquitetura e fluxo do sistema
